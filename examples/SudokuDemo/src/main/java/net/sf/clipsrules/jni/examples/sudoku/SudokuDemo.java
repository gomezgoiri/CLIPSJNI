...
public class SudokuDemo implements ActionListener, FocusListener, KeyListener {
	JFrame jfrm;
	JPanel mainGrid;
	...
	SudokuDemo() throws CLIPSError, FileNotFoundException {
		JTable theSubGrid;
		int r, c;
		...
		try {
			this.sudokuResources = ResourceBundle.getBundle(
					"properties.SudokuResources", Locale.getDefault());
		...
		this.mainGrid.setLayout(theLayout);
		...
		for (r = 0; r < 3; r++) {
			for (c = 0; c < 3; c++) {
				...
				theSubGrid.setBorder(BorderFactory.createLineBorder(
						Color.black, 2));
		...
		this.clearButton = new JButton(sudokuResources.getString("Clear"));
		this.clearButton.setActionCommand("Clear");
		buttonGrid.add(this.clearButton);
		...
		buttonGrid.add(this.resetButton);
		...
		buttonGrid.add(this.solveButton);
		...
		buttonGrid.add(this.techniquesButton);
		this.techniquesButton.addActionListener(this);
		this.techniquesButton.setToolTipText(this.sudokuResources
				.getString("TechniquesTip"));
		...
		mainPanel.add(this.mainGrid);
		mainPanel.add(buttonGrid);
		this.jfrm.getContentPane().add(mainPanel, BorderLayout.NORTH);

		final JLabel instructions = new JLabel("<html><p style=\"font-size:95%\">"
				+ this.sudokuResources.getString("Instructions") + "</p><br>");
		final JPanel labelPanel = new JPanel();
		labelPanel.setLayout(new FlowLayout());
		labelPanel.add(instructions);
		this.jfrm.getContentPane().add(labelPanel, BorderLayout.SOUTH);
		...
		this.clips.load( getFilePath("sudoku.clp") );
		this.clips.load( getFilePath("solve.clp") );
	...
	public void onActionPerformed(ActionEvent ae) throws Exception {
			...
			for (int i = 0; i < 9; i++) {
				...
				for (int r = 0; r < 3; r++) {
					for (int c = 0; c < 3; c++) {
						...
						assertStr = "(possible (row "
								+ (r + (rowGroup * 3) + 1) + ") " + "(column "
								+ (c + (colGroup * 3) + 1) + ") " + "(group "
								+ (i + 1) + ") " + "(id "
								+ ((i * 9) + (r * 3) + c + 1) + ") ";
		...
		/* =============================== */
			...
			JOptionPane.showMessageDialog(jfrm, messageStr,
					sudokuResources.getString("SolutionTechniques"),
